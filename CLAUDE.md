...
5. **Output** - `output_formatter.py` generates JSON and Markdown; HTML output (if `ENABLE_HTML_OUTPUT=true`) is generated by `set_explorer_formatter.py` (set detail), `artist_explorer_formatter.py` (artist summary), and `explorer_formatter.py` (master index) — all saved to `output/<mix_name>/`
